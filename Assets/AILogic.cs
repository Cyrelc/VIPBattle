...
public class AILogic : MonoBehaviour {
    ...
    public float groupDirectionWeight = 0.025f;     //the weight to place on what direction the group is moving in
    public float groupDirectionRadius = 6f;         //the radius within which to detect what direction your allies are moving

    public float groupCohesionWeight = 1f;          //the weight to place on staying with the group. This will likely decrease over time while in searching mode
    public float groupCohesionRadius = 6f;          //the radius in which to check the location of your allies, this may increase over time while in searching mode
    ...
    void Update() {
        ...
        Vector3[] combinedHeadings = { toFriends, toEnemies, awayFromWalls, toFriendlyVIP, toEnemyVIP, needHelp };
        float[] combinedWeights = { friendWeight, enemyWeight, wallWeight, friendlyVIPWeight, enemyVIPWeight, helpWeight };
